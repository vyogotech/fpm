...
FPM uses a configuration file located at `~/.fpm/config.json` to store settings such as repository configurations and default behaviors. This file is automatically created by FPM on its first run if it doesn't exist.

One key configuration option relevant to packaging is `default_package_exclusions`. This is a list of file and directory patterns that are excluded by default when creating "prod" type packages (see `fpm package` documentation for more details on exclusions). Users can customize this list to suit their needs.
...
# Or, rely on version from hooks.py:
# fpm package --org myorg
...
The version for the package is determined in the following order of precedence:
1.  The `--version` flag provided to the `fpm package` command.
2.  The `app_version` variable defined in the app's `hooks.py` file (e.g., `app_version = "1.0.0"`).
3.  The `__version__` variable defined in the app's `hooks.py` file (if `app_version` is not found).
If no version can be determined through any of these means, the command will return an error.

**File Exclusions:**

When packaging, FPM excludes files based on several layers:
1.  **Minimal Hardcoded Patterns**: A very small set of patterns like `.git/` are always excluded.
2.  **Global Exclusions (for `prod` type)**: The `default_package_exclusions` list in your `~/.fpm/config.json` file is applied when using `--package-type prod` (which is the default). Users can customize this list. FPM pre-populates this on first config generation with common patterns such as:
    *   Python artifacts: `__pycache__/`, `*.pyc`, `build/`, `dist/`, `*.egg-info/`
    *   Common OS files: `.DS_Store`, `Thumbs.db`
    *   Editor directories: `.vscode/`, `.idea/`
    *   Testing and development files: `tests/`, `test/`, `node_modules/`, `compose.yml`, `docker-compose.yml`, `.devcontainer/`
3.  **App-Specific Exclusions**: Patterns defined in an `.fpmignore` file in the root of your app source directory are always applied. This file uses gitignore syntax.

**Interaction with `--package-type` flag:**
*   `fpm package --package-type prod` (or by default): Applies all exclusion layers: minimal hardcoded + global `default_package_exclusions` from `config.json` + `.fpmignore`. This is suitable for production releases.
*   `fpm package --package-type dev`: Applies only the minimal hardcoded exclusions and `.fpmignore`. It *skips* the global `default_package_exclusions` from `config.json`, potentially including more files useful for development or testing.
